...
class GNN(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if pred_head == "cls":
            ...
            self.graph_last_activate= torch.nn.Sequential(nn.Linear(emb_dim, 2*emb_dim), nn.ReLU())
            self.graph_pred_linear = torch.nn.Linear(2 * self.emb_dim, self.num_class)
            ...
            self.graph_pred_linear = torch.nn.Sequential(nn.Linear(emb_dim, 2*emb_dim), nn.ReLU(), 
                                                         nn.Linear(2 * self.emb_dim, self.num_class))
    ...
    def forward(self, batched_data, get_rep=False):
        ...
        h_graph =self.graph_last_activate(h_graph) if self.graph_last_activate is not None else h_graph
        if get_rep:
    ...
    def get_w(self):
        if self.pred_head == 'inv' or 'cls':
            weight = self.graph_pred_linear.weight
        else:
            raise ValueError('wrong pred_head'+self.pred_head)
        return weight
