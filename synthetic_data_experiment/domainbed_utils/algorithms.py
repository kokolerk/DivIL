...
from domainbed_utils.lib.misc import (
...
class Algorithm(torch.nn.Module):
    ...
    def update(self, minibatches, unlabeled=None, use_ucl=False, factor=0.1):
        """
    ...
    def _augmentation(self, x, noise=0.1):
        batch_size = x.size()[0]
        x1 = x + torch.cat([torch.zeros(batch_size, 8), torch.randn(batch_size, 8) * noise], dim=1)
        x2 = x + torch.cat([torch.zeros(batch_size, 8), torch.randn(batch_size, 8) * noise], dim=1)
        return x1, x2
    ...
    def _calculate_infoNCE_loss(self, x, factor=1):
        x1, x2 = self._augmentation(x)
        z1 = self.featurizer(x1)
        z2 = self.featurizer(x2)
        ucl_loss = self.InfoNCE_loss(z1, z2)
        return ucl_loss * factor
...
class IRM(ERM):
    ...
    def update(self, minibatches, unlabeled=None, use_ucl=False, factor=1):
        device = "cuda" if minibatches[0][0].is_cuda else "cpu"
        ...
        # additional
        if use_ucl:
            ucl_loss = self._calculate_infoNCE_loss(all_x, factor=factor)
            loss += ucl_loss
...
class VREx(ERM):
    ...
    def update(self, minibatches, unlabeled=None, use_ucl=False, factor=1):
        ...
        if use_ucl:
            loss += self._calculate_infoNCE_loss(x, factor=factor)
