...
class CIGA(nn.Module):
    ...
    def get_w(self):
        '''for neural collapse calculation'''
        return self.classifier.get_w


class CIGA_ssl(nn.Module):
    '''contrast postive samples from edge attention
    ratio1: subgraph ratio for sample1,
    ratio2: subgraph ratio for sample2,
    -1 is for identical graph
    return causal rep1, causal rep2, pred1, pred2
    '''
    def __init__(self,
                 input_dim,
                 out_dim,
    ...
    def forward(self, batch, return_data="pred", return_spu=False, debug=False):
        ...
        # subgraph1
        if self.ratio1<0:
            (causal_edge_index, causal_edge_attr, causal_edge_weight1), \
                (spu_edge_index, spu_edge_attr, spu_edge_weight) = (batch.edge_index, batch.edge_attr, pred_edge_weight), \
                (batch.edge_index, batch.edge_attr, pred_edge_weight)
        else:
            (causal_edge_index, causal_edge_attr, causal_edge_weight1), \
                (spu_edge_index, spu_edge_attr, spu_edge_weight) = self.split_graph(batch, pred_edge_weight, self.ratio1)
        ...
        # obtain \hat{G_c}
        causal_graph1 = DataBatch.Batch(batch=causal_batch,
                                       edge_index=causal_edge_index,
                                       x=causal_x,
                                       edge_attr=causal_edge_attr)
        ...
        # subgraph 2
        if self.ratio2<0:
            (causal_edge_index, causal_edge_attr, causal_edge_weight2), \
                (spu_edge_index, spu_edge_attr, spu_edge_weight) = (new_batch.edge_index, new_batch.edge_attr, pred_edge_weight), \
                (new_batch.edge_index, new_batch.edge_attr, pred_edge_weight)
        else:
            (causal_edge_index, causal_edge_attr, causal_edge_weight2), \
                (spu_edge_index, spu_edge_attr, spu_edge_weight) = self.split_graph(new_batch, pred_edge_weight, self.ratio2)


        if self.c_in.lower() == "raw":
            causal_x, causal_edge_index, causal_batch, _ = relabel(new_batch.x, causal_edge_index, new_batch.batch)
            spu_x, spu_edge_index, spu_batch, _ = relabel(new_batch.x, spu_edge_index, new_batch.batch)
        else:
            causal_x, causal_edge_index, causal_batch, _ = relabel(h, causal_edge_index, new_batch.batch)
            spu_x, spu_edge_index, spu_batch, _ = relabel(h, spu_edge_index, new_batch.batch)
        
        # obtain \hat{G_c}
        causal_graph2 = DataBatch.Batch(batch=causal_batch,
                                       edge_index=causal_edge_index,
                                       x=causal_x,
                                       edge_attr=causal_edge_attr)

        set_masks(causal_edge_weight1, self.classifier)
        # obtain predictions with the classifier based on \hat{G_c}
        causal_pred1, causal_rep1 = self.classifier(causal_graph1, get_rep=True)
        clear_masks(self.classifier)

        set_masks(causal_edge_weight2, self.classifier)
        # obtain predictions with the classifier based on \hat{G_c}
        causal_pred2, causal_rep2 = self.classifier(causal_graph2, get_rep=True)
        clear_masks(self.classifier)
        ...
        if return_data.lower() == "pred":
            ...
            return causal_pred1, causal_rep1
        elif return_data.lower() == "ssl" :
            return causal_pred1, causal_rep1, causal_pred2, causal_rep2
        elif return_data.lower() == "feat":
            causal_h, _, __, ___ = relabel(h, causal_edge_index, batch.batch)
            if self.c_pool.lower() == "add":
                casual_rep_from_feat = global_add_pool(causal_h, batch=causal_batch)
            ...
            return (causal_x, causal_edge_index, causal_edge_attr, causal_edge_weight1, causal_batch),\
                (spu_x, spu_edge_index, spu_edge_attr, spu_edge_weight, spu_batch),\
                pred_edge_weight
...
if __name__ == "__main__":
    ciga = CIGA_ssl(input_dim=1, out_dim=1)
